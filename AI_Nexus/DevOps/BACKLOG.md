...
| B-026 | Identity: Persistent GUID Serialization Hardening | 5 | Todo | DevOps | Prevent 'ApplyStateFromJsonObject' from overwriting unique top-level EntityIDs with stale/duplicate nested JSON GUIDs during world state reconstitution. | Deserialization prioritizes the top-level registry-assigned FGuid and protects it from colliding or stale nested GUID properties. |
