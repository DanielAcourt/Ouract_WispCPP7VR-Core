...
| B-027 | AAS v1.4.0 Hardening | 3 | Active Sprint | Research | Refactor the hardcoded diligence score to dynamically verify backup files on disk. | Diligence score calculates actual .bak coverage ratios dynamically. |
...
| AD-005a | Spatial Sense: Save State Ingestion | 5 | Done | Research/DevOps | Ingest the serialized component save packet during `/v1/unreal/chat` calls. | Map serialized JSON save payloads to the LLM's system context prompt framing. |
...
| B-031 | Hardware-Agnostic Telemetry Fallback | 5 | Active Sprint | Research/DevOps | Decouple the T-pillar from static nvidia-smi calls to support standard development setups. | Implement the Strategy Pattern to dynamically find and fallback to AMD, Intel, and CPU/RAM psutil sensors. |
